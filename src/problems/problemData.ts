...
public class Main {
  ...
  variable: {
    instrumented: `
s.set('x', <1-5>);
const t = new Turtle(s.get('x'), <1-5>);
t.forward();
 `.trim(),
    java: `
public class Main {
...
s.set('x', s.get('x') + 1);
s.set('y', s.get('x') + 1);
const t = new Turtle(s.get('x'), s.get('y'));
t.forward();
`.trim(),
    java: `
public class Main {
...
s.set('y', s.get('x') * 2);
s.set('y', s.get('y') / 3);
const t = new Turtle(s.get('x') + 1, s.get('y') + 1);
t.forward();
`.trim(),
    java: `
public class Main {
...
s.set('b', s.get('a') * 2);
s.set('c', s.get('b') - 2);
const t = new Turtle(s.get('c'), s.get('b'));
t.forward();
`.trim(),
    java: `
public class Main {
...
delete s.vars['i'];
s.set('x', s.get('x') / 3);
const t = new Turtle(s.get('x') + 1, 0);
t.forward();
    `.trim(),
    java: `
public class Main {
...
for (const cmd of ['ri', 'add', 'fo', 'add', 'le', 'fo', 'fo']) {
  s.set('cmd', cmd);
  call(parse, 't', 'cmd', 'x')(t, s.get('cmd'), s.get('x'));
  if (cmd === 'add') {
    s.set('x', s.get('x') + 1);
  }
}
...
function parse(t, c, x) {
  if (c === 'fo') call(forwardGivenSteps, 't', 'x')(t, x);
  else if (c === 'ri') t.turnRight();
