    ...
    <VStack align="stretch" spacing={6}>
      ...
      <Box>
        ...
        <Text>
          ...
          」の2種類があり、すべての問題は一括実行モードから始まります。一括実行モードの方がステップ実行モードよりも難しいです。
          一括実行モードで3回間違えると、自動的により簡単なステップ実行モードに切り替わります。
          どちらのモードでも、解答者はタートル（亀）を操作して、プログラムの実行後の盤面を再現し、変数の値を入力する必要があります。
          一括実行モードで正解するか、もしくは、ステップ実行モードのすべてのステップで正解すれば、問題を解き終えた（完了）と記録されます。
          完了状態に関する詳細は、ページ下部に記載している「完了のチェックマークがつくまで問題を解く」で確認してください。
        </Text>
        ...
        <List my={2}>
          <ListItem>
            ①問題に正解すると、問題一覧ページの各問題の先頭に、完了のチェックマークがつき、初回の完了日時が表示されます。これらが表示されるまで問題を解き進めてください。
          </ListItem>
          <ListItem>②「未完了」と表示されている問題は0点扱いとなります。</ListItem>
        </List>
