...
# .docker.env holds only the non-secret values of the selected fnox profile (`wb gen-docker-env`);
...
# Only node is installed, not every tool mise.toml pins: fnox.toml is deliberately kept out of the
# build context and the age key never enters the image, so the container can never use fnox.
