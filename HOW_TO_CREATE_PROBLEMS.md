...
`java` と `instrumented` の両方で、空欄にしたい箇所を `@[模範解答]@` で囲む。空欄は式または文の単位で作り、両言語で同じ順序・同じ個数にする。例:
...
`java` に `@[...]@` が含まれる問題は自動的に穴埋め問題として扱われ、実行後の盤面と変数の値を見せた上で空欄の入力を求める。`java` 側は実際にコンパイル・実行されることがあるため、正しい Java プログラムにすること。

採点は次の順で行う。段階 1 で一致した解答と、段階 2 で盤面・亀・変数のいずれかが異なると判定された解答はそこで確定する。段階 2 で全て一致した解答は、Java として正しいことを確認するために段階 3・4 でも実行する。

1. 模範解答との文字列一致（空白の違いは無視）。
2. 解答を `instrumented` の記法に変換して既存の JavaScript トレーサで再実行し、最終的な盤面・亀・変数を比較する。変換できない記法や、変換結果が JavaScript として実行できない場合は段階 3 に進む。変換できるのは四則演算（除算を除く）、比較、論理演算、代入、`++`/`--`、亀のメソッド呼び出し、`new Turtle(...)` など限られた記法のみ。
3. Wandbox で Java プログラムとして実行し、最終的な盤面と亀を比較する。
4. Wandbox が利用できない場合は、[judge](https://github.com/WillBooster/judge) で実行する。judge は自身のマシン上でサンドボックス化して Java プログラムをコンパイル・実行する。

穴埋め問題も他の問題と同様に必ず亀を使うこと。段階 3・4 は最終的な盤面と亀の状態で採点するため、亀のいないプログラムは採点できない。
