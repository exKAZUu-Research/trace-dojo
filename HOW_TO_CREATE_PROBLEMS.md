...
採点は次の順で行い、前の段で判定できた場合は後の段に進まない。

1. 模範解答との文字列一致（空白の違いは無視）。
2. 解答を `instrumented` の記法に変換して既存の JavaScript トレーサで再実行し、最終的な盤面・亀・変数を比較する。変換できるのは四則演算（除算を除く）、比較、論理演算、代入、`++`/`--`、亀のメソッド呼び出し、`new Turtle(...)` など限られた記法のみ。
3. 変換できない解答は Wandbox で Java プログラムとして実行し、最終的な盤面と亀を比較する。
